...
void user_save_money::on_close_music_clicked()
{
    Widget w;
    //w.sound->stop();
}
...
void user_save_money::on_yesorno_clicked(QAbstractButton *button)
{
    ...
    str.first_money = this->ui->save_money->text();

    if(this->ui->yesorno->button(QDialogButtonBox::Ok) == button)           //判断按下的是否为"确定”按钮
    {
        if((this->ui->user_num->text() == "") || (this->ui->user_id->text() == "") || (this->ui->user_passward1->text() == "") || (this->ui->user_passward2->text() == "") || (this->ui->save_money->text() == ""))
        {
        ...
        switch (message.cmd)
        {
            ...
            {
                QMessageBox::information(this, tr("消息"), "存款成功!", QMessageBox::Ok);
                save_data();                                    //保存本地
                this->close();
                deal_user D(this);
                D.show();
...
void user_save_money::save_data()
{
    ...
    QString save = QString("\n%1\t%2\t0.00\t0.00\t+%3\t\t%4\t\t%5").arg(current_date).arg(str.first_money).arg(str.first_money).arg(message.money).arg(yyy);
    qDebug()<<save;
