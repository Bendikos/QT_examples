...
public:
    ...
    void PlaySound(const QString &soundName)
    {
        ...
        // 设置声音源路径
        soundEffect->setSource(QUrl::fromLocalFile(soundName));

        // 播放声音
        soundEffect->play();

        // 确保 soundEffect 对象在播放期间不会被销毁
        // 在实际应用中你可能需要管理 soundEffect 的生命周期
    }
...
public:
    ...
    static void FormQQ(QWidget *frm,int frmHeight)
    {
        if (frm->windowState()!=Qt::WindowMinimized)
        {
            ...
            int DeskWidth=screen->geometry().width();
    ...
    {
         QScreen *screen = QGuiApplication::primaryScreen();
        int screenX=screen->geometry().width();
        int screenY=screen->geometry().height();
        int wndX=frm->width();
        int wndY=frm->height();
    ...
    {
        QScreen *screen = QGuiApplication::primaryScreen();
        return QSize(screen->availableGeometry().width(),screen->availableGeometry().height());
    }
